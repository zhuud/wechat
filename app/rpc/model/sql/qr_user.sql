...
(
    ...
    `skip_verify`     tinyint(1) NOT NULL DEFAULT 1 COMMENT '外部客户添加时是否无需验证,0-否 1-是',
    ...
    `is_temp`         tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否临时会话模式0 不是 1 是',
    ...
    `is_exclusive`    tinyint(1) NOT NULL DEFAULT 0 COMMENT '0-否 1-是；是否开启同一外部企业客户只能添加同一个员工，默认为否，开启后，同一个企业的客户会优先添加到同一个跟进人',
