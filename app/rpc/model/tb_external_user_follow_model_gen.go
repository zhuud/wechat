...
type (
	tbExternalUserFollowModel interface {
		...
		FindOne(ctx context.Context, seq uint64) (*TbExternalUserFollow, error)
	...
	TbExternalUserFollow struct {
		...
		AddWay            uint64    `db:"add_way"`             // 添加外部联系人的方式(https://developer.work.weixin.qq.com/document/path/92114#%E6%9D%A5%E6%BA%90%E5%AE%9A%E4%B9%89) | 2020-09-10
...
func (m *defaultTbExternalUserFollowModel) Delete(ctx context.Context, seq uint64) error {
...
func (m *defaultTbExternalUserFollowModel) FindOne(ctx context.Context, seq uint64) (*TbExternalUserFollow, error) {
	tbExternalUserFollowSeqKey := fmt.Sprintf("%s%v", cacheTbExternalUserFollowSeqPrefix, seq)
...
func (m *defaultTbExternalUserFollowModel) Insert(ctx context.Context, data *TbExternalUserFollow) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.ExternalUserid, data.Unionid, data.Userid, data.Platform, data.OperUserid, data.AddWay, data.State, data.StateChannel, data.StateChannelValue, data.Remark, data.RemarkMobiles, data.Description, data.RemarkCorpName, data.RemarkPicMediaid, data.Status)
	}, tbExternalUserFollowSeqKey)
	return ret, err
}
...
func (m *defaultTbExternalUserFollowModel) Update(ctx context.Context, data *TbExternalUserFollow) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.ExternalUserid, data.Unionid, data.Userid, data.Platform, data.OperUserid, data.AddWay, data.State, data.StateChannel, data.StateChannelValue, data.Remark, data.RemarkMobiles, data.Description, data.RemarkCorpName, data.RemarkPicMediaid, data.Status, data.Seq)
	}, tbExternalUserFollowSeqKey)
	return err
}
